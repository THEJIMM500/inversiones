...
public class ConexionUsuarios {
		...
		private final String campoContraseña = "contrasena";
		private final String campoNombre = "nombre";
		private final String campoApellido = "apellidos";
		private final String campoNtarjeta = "ntarjeta";
		private final String campoDirecion = "direccion";
		...
		private final String campoValidado = "validado";
		
	public ConexionUsuarios() throws SQLException {
		String user = "root";
		String password = "";
	...
	public boolean registrarUsuario(String nombre, String contraseña) throws SQLException {
		if(exiteUsuario(nombre)== false) {
			String sentencia = "INSERT INTO clientes (usuario, "+campoContraseña+") values ('"+nombre+"', '"+contraseña+"');";
	...
	public int iniciarSesion(String nombre, String contraseña) throws SQLException {		
		if(exiteUsuario(nombre)==true) {
			...
			if(nombreLeido.equals(nombre)&&contraseñaLeida.equals(contraseña)) {
				resultado.close();
				canuto.close();
				return 0;//valor que regresa si sea iniciado sesion
			}else {
				resultado.close();
				canuto.close();
				return 1; //valor que regresa si los datos no coinciden
			}
		}
	...
	public String getTelefono(String Usuario) throws SQLException {
		String sentencia = "SELECT `"+campoTelefono+"` FROM `clientes` WHERE `"+campoUsuario+"` = \""+Usuario+"\";";
		Statement canuto = Conector.createStatement();
		ResultSet resultado = canuto.executeQuery(sentencia);
		resultado.next();
	...
	public void setSaldo(String Usuario, double Saldo) throws SQLException {
		String sentencia = "UPDATE `clientes` SET `"+campoSaldo+"`=\""+Saldo+"\" WHERE `"+campoUsuario+"`= \""+Usuario+"\";";
		Statement canuto = Conector.createStatement();
		canuto.executeUpdate(sentencia);
		canuto.close();
	}
	
	public double getSaldo(String Usuario) throws SQLException {
		String sentencia = "SELECT `"+campoSaldo+"` FROM `clientes` WHERE `"+campoUsuario+"` = \""+Usuario+"\";";
		Statement canuto = Conector.createStatement();
		ResultSet resultado = canuto.executeQuery(sentencia);
		resultado.next();
		double valorDevolver=resultado.getDouble(campoSaldo);
		resultado.close();
		canuto.close();
		return valorDevolver;
	}
	
	public void validar(String Usuario) throws SQLException {
		String sentencia = "UPDATE `clientes` SET `"+campoValidado+"`=\""+1+"\" WHERE `"+campoUsuario+"`= \""+Usuario+"\";";
		Statement canuto = Conector.createStatement();
		canuto.executeUpdate(sentencia);
		canuto.close();
	}
	
	public int getValidado (String Usuario) throws SQLException {
		String sentencia = "SELECT `"+campoValidado+"` FROM `clientes` WHERE `"+campoUsuario+"` = \""+Usuario+"\";";
		Statement canuto = Conector.createStatement();
		ResultSet resultado = canuto.executeQuery(sentencia);
		resultado.next();
		int valorDevolver=resultado.getInt(campoValidado);
		resultado.close();
		canuto.close();
		return valorDevolver;
	}
